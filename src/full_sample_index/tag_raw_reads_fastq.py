...
def process_file(file):
    ...
    with open(file, "r") as old_handle:
        ...
        for i, batch in enumerate(batch_iterator(record_iter, 1000000)):
            ...
            count = SeqIO.write(all_sequences, new_handle, "fastq")
            print(
                f"extracted {count} sequences from {file.split('/')[-1]} into batch {i}"
            )
            new_handle.close()


pool = multiprocessing.Pool(48)
work = pool.map(process_file,
                [f"{file}" for file in glob(f"{source_dir}/*.fasta")])
